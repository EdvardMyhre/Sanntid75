	...
	for{
		select{
			...
			if network_message_receive.Message_type == Network.MESSAGE_TYPE_REQUEST_WEIGHT {
				...
				network_message_send.Message_type = types.MESSAGE_TYPE_GIVE_WEIGHT
			...
			} else if network_message_receive.Message_type == types.MESSAGE_TYPE_DISTRIBUTE_ORDER {
...
func Test_distributor_taskmanagerEmulator(send_chan chan types.Task, receive_chan chan types.Task){
	...
	for{
		if(time.Since(time_duration) > 5*time.Second) {
			internal_message_test_to_distributor.Type = types.MESSAGE_TYPE_DISTRIBUTE_NEWORDER
